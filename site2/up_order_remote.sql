...
BEGIN
    IF Y=0 
        THEN
            ...
            END IF;
        ELSIF Y=1 THEN
            SELECT COUNT(o_id) INTO oid FROM orders@dhaka WHERE o_id=Z;
            IF oid>0
                THEN
                    SELECT prod_id,o_unit INTO pid,punit FROM orders@dhaka WHERE o_id=Z;
                    UPDATE product SET prod_unit=prod_unit+punit WHERE prod_id=pid;
                    DELETE orders@dhaka WHERE o_id=Z;    
            END IF;                    
    END IF;
