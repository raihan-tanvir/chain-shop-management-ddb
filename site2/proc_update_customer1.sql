...
BEGIN 
    ...
    IF A!=0 THEN
        ...
        ELSIF choice=2 THEN
            ...
            IF ccity='Dhaka' THEN
                INSERT INTO customer@dhaka SELECT * FROM customer WHERE cus_id=cid;
        ...
        ELSE
            ...
            IF A!=0 THEN
                IF choice=1 THEN
                    UPDATE customer@dhaka SET cus_name=cname WHERE cus_id=cid;
                ELSIF choice=2 THEN
                    UPDATE customer@dhaka SET city=ccity WHERE cus_id=cid;    
                    INSERT INTO customer SELECT * FROM customer@dhaka WHERE cus_id=cid;
                    DELETE FROM customer@dhaka WHERE cus_id=cid;
                                        
                ELSIF choice=3 THEN
                    UPDATE customer@dhaka SET address=cadd WHERE cus_id=cid;		
                END IF;     
             END IF;         
    END IF;
